namespace Aneejian.PowerPoint.Downsizer
{
    public static class Fickles
    {
        public const string ClassLibraryName = "PowerPoint Downsizer";
        public const string AddInName = "PowerPoint Downsizer Add-In";
        public const string WebAddInName = "PowerPoint Downsizer Web Add-In";
        public const string AddInDescription = "Add-In to downsize PowerPoint presentations by removing unused layouts and master slides.";
        public const string NothingToRemove = "Nothing to remove. All layouts and master slides in use.";

        public const string HideTabLabel = "Hide the Downsizer tab.";
        public const string UnhideTabLabel = "Show the Downsizer tab.";

        public const string BuyCoffeeUrl = "https://www.buymeacoffee.com/aneejian";
        public const string HelpUrl = "https://kannansuresh.github.io/PowerPoint-Downsizer/";
    }
}